...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # First affine layer (after conv - relu - pool) parameters
        # Assuming the use of a 2x2 pool with stride 2, the dimensionality is reduced by a factor of 2
        pool_height, pool_width, pool_stride = 2, 2, 2
        H_affine = (H // pool_height) * (W // pool_width) * F
        self.params['W2'] = weight_scale * np.random.randn(H_affine, hidden_dim)
        self.params['b2'] = np.zeros(hidden_dim)

        # Second affine layer (output layer) parameters
